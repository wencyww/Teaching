...
set more off
cap log close
cd "O:\wang6054\PA 5033 Spring 2022\Lab 3&4"
...
*A: With one dollar increase in the average wage, the employment proportion would decrease by 4.1%; similarly, with 1% decrease in the employment proportion, the average wage would decrease by 11.1 dollars. Both of them are significant at 1% level, and they meet with the demand-side economic theory.

*Q: What might be wrong with this model?		
*A: Nothing LOOKS wrong, but we should expect simultaneity
	* Probably overinflated t-scores
	* Means maybe won't actually be as significant as seems
...
*A: We DO see some changes from original model!
	* Estimates are STILL biased, but it is LESS biased
		* If the instrument is poor, this will not  be a better model
	* Increased variance and std. errors
		* Meaning things look less significant (good)
...
/*A: If you try to run an ivreg with more regressors in the full equation 
(second stage) than the first stage (instrumented) regression, Stata will 
bark at you and not run properly. This violates the order condition, 
which essentially says that you have to have at least as many variables being 
instrumented as there are in the full equation. This is only a problem if any 
of the variables being used in the instrument are also in the full equation, 
like our case now. ***/
